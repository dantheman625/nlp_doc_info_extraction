...
**3. NER Evaluation**  
The evaluation process aligns the predicted entities with the ground truth annotations from the evaluation dataset. It converts the character-based spans of predicted entities into token-level BIO tags to ensure compatibility with standard evaluation formats. The `seqeval` library is used to compute precision, recall, and F1 scores, which are logged to Weights & Biases (wandb) for experiment tracking and comparison.

**4. RE Candidate Generation**  
Candidate relation pairs are generated by first splitting the evaluation dataset into individual sentences using the NLTK tokenizer. For each sentence, the previously predicted NER entities are used to identify all possible candidate entity pairs. These pairs are assembled into a dataset containing the sentence text, the two entities involved, and placeholder relation labels. The dataset is then converted into a Hugging Face `Dataset` object to prepare it for relation classification.

**5. RE Model Loading and Inference**  
The selected RE model and tokenizer are loaded and initialized as a Hugging Face text classification pipeline. This pipeline processes the candidate entity pairs and predicts relation types along with confidence scores for each pair. The resulting predictions are saved in a JSON file for further evaluation and integration into the full pipeline.

**6. RE Evaluation**  
The predicted relation labels are mapped to the ground truth labels using a predefined schema (`mapping_challenge_to_docred`). Evaluation metrics—including precision, recall, and F1 score are calculated using `sklearn.metrics`, and the results are logged to wandb. Additionally, a detailed classification report is generated with per-relation performance and saved as a JSON file for analysis.

**7. Combined NER + RE Pipeline**  
In this step, the outputs of the NER and RE models are merged to generate structured information per document. Each output includes the original document text, the extracted named entities, and the predicted relationships connecting those entities. This forms the core output format of the full information extraction pipeline.

**8. Logging and Visualization**  
Key metrics for both NER and RE tasks are logged to wandb, including precision, recall, and F1 scores. A summary comparison table is created within the wandb dashboard to facilitate model performance tracking. Additionally, the evaluation can be filtered to exclude "no_relation" predictions, focusing instead on performance for positive relation instances.

**9. Final Outputs**  
The final structured results—including both predictions and evaluation summaries—are saved as JSON files to the `data/processed` directory. The wandb session is finalized, and a summary of key metrics and outputs is printed for review.

## Setup Instructions

1. **Download Project Files**
Go to this [link](https://drive.google.com/drive/folders/1_PLgeIXWbtGyZJCZsXuJNUzjqM1EiIlw?usp=share_link) and download the folder. Put the folder to the project root.
